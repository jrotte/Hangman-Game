...
var gameObject = {
	...
	athsArray: ["JORDAN", "PIPPEN", "RODMAN", "KERR", "HALEY", "HARPER", "LONGLEY", "SALLEY", "KUKOC"],
	randomWord: "",
	athLetters:[],
	...
	generateWord: function(){
		...
		this.randomWord = this.athsArray[random_num];
		this.athLetters = this.randomWord.split("");

		console.log(this.randomWord + " " + this.athLetters);
	...
	checkMatch: function(){
		...
		for (var i=0; i < this.athLetters.length; i++){
			if (this.currentLetter == this.athLetters[i]){
				matchCounter++;
			}
		}
	...
	revealath: function(){
		...
		if (this.correctGuesses.length == 0){
			...
			for (var i=0; i<this.athLetters.length; i++){
				...
				if (this.correctGuessesInOrder[i] != this.athLetters[i]){

					for (var j=0; j<this.correctGuesses.length; j++){

						if (this.correctGuesses[j] == this.athLetters[i]){
							this.correctGuessesInOrder[i] = this.athLetters[i];
						}
	...
	checkProgress: function(){
		...
		if (counter == this.athLetters.length){
			alert("You win");
			this.winCount++;
			this.generateWord();
		}
